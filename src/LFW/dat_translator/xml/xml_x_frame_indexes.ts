...
export function xml_x_frame_indexes(xml: IXML, indexes: IFrameIndexes | undefined, tag: string): IXMLElement | undefined {
  ...
  ret.set_arr_attr("in_the_skys", indexes.in_the_skys);
  ret.set_arr_attr("throwings", indexes.throwings);
  ret.set_arr_attr("on_hands", indexes.on_hands);
  ret.set_arr_attr("falling_1", indexes.falling?.[1]);
  ret.set_arr_attr("falling_2", indexes.falling?.[-1]);
  ret.set_arr_attr("bouncing_1", indexes.bouncing?.[1]);
  ret.set_arr_attr("bouncing_2", indexes.bouncing?.[-1]);
  ret.set_arr_attr("critical_hit_1", indexes.critical_hit?.[1]);
  ret.set_arr_attr("critical_hit_2", indexes.critical_hit?.[-1]);
  ret.set_attr("injured_1", indexes.injured?.[1]);
  ret.set_attr("injured_2", indexes.injured?.[-1]);
  ret.set_arr_attr("grand_injured_1", indexes.grand_injured?.[1]);
  ret.set_arr_attr("grand_injured_2", indexes.grand_injured?.[-1]);
  ret.set_attr("lying_1", indexes.lying?.[1]);
  ret.set_attr("lying_2", indexes.lying?.[-1]);
  ret.set_arr_attr("fire", indexes.fire);
