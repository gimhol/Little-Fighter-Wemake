...
export function xml_x_itr(xml: IXML, i: IItrInfo, tag: string): IXMLElement {
  ...
  xml_from_t_next_frame(xml, i.catchingact, 'catchingact').forEach(v => ret.insert(v))
  xml_from_t_next_frame(xml, i.caughtact, 'caughtact').forEach(v => ret.insert(v))
  xml_from_t_next_frame(xml, i.on_hit_ground, 'on_hit_ground').forEach(v => ret.insert(v))
